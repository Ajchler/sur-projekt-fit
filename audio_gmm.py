...
def read_dataset_n(dataset, pipeline, n=1):
    # Read dataset N times (to apply different augmentations)
    res = []
    for _ in range(n):
        res.append(read_dataset(dataset, pipeline))
    return np.vstack(res)

def train_gmm(target, tN, non_target, ntN):
    t_gmm = GaussianMixture(n_components=tN)
    nt_gmm = GaussianMixture(n_components=ntN)
    print("Fitting target GMM")
    t_gmm.fit(target)
    print("Fitting non-target GMM")
    nt_gmm.fit(non_target)
    return t_gmm, nt_gmm

def save_gmm(t_gmm, nt_gmm, dir='.', path='audio_classifier.pkl'):
    if not os.path.exists(dir):
        os.makedirs(dir)
    if dir[-1] != '/':
        dir += '/'
    GMM_file = open(dir + path, "wb")
    pkl.dump((t_gmm, nt_gmm), GMM_file)
    GMM_file.close()
...
if __name__ == "__main__":
    ...
    n_splits = 5     # number of splits to do in stratified shuffle split
    train_size = 0.6 # train size
    train_read_n = 4 # how many times training data is being read (applying different augmentations)
    test_read_n = 2  # how many times test data is being read (applying different augmentations)
    tN = 15           # target components (GMM)
    ntN = 30         # non-target components (GMM)
    dir=f"models/t{tN}_n{ntN}_tread{train_read_n}_ntread{test_read_n}" # where to store models
    train_pipeline = Pipeline(effects=aug_effects) # training augmentation pipeline
    ...
    overall_acc = [0, 0, 0, 0] # default correct target, default correct non-target, augmented correct target, augmented correct non-target
    for i, (train_index, test_index) in enumerate(sss.split(dataset, labels)):
        ...
        val_t_idx = test_index[np.argwhere(labels[test_index] == 1)].ravel()
        val_nt_idx = test_index[np.argwhere(labels[test_index] == 0)].ravel()

        # Read the training data multiple times using the train_pipeline to apply different augmentations
        train_t = read_dataset_n(Subset(dataset, train_t_idx), train_pipeline, train_read_n)
        train_nt = read_dataset_n(Subset(dataset, train_nt_idx), train_pipeline, test_read_n)
        print(f"Train target shape: {train_t.shape}, train non-target shape: {train_nt.shape}")

        t_gmm, nt_gmm = train_gmm(train_t, tN, train_nt, ntN)
        save_gmm(t_gmm, nt_gmm, dir=dir, path=f"gmm_{i}.pkl")

        val_t = Subset(dataset, val_t_idx)
        val_nt = Subset(dataset, val_nt_idx)
